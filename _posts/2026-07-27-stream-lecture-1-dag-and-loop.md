...
选择 push 的原因很直接。输入无界，根节点没有"查询结束"这一刻：拉模型的线程会永远阻塞在 source 的空等上；push 让线程在没数据时睡觉，数据到达时被唤醒。输入无界之后，火山模型里原本不需要单独操心的三件事，都变成了需要专门处理的负担：

| 在火山模型里不需要单独操心 | 无界输入下的下场 | 流系统的应对 |
|---|---|---|
