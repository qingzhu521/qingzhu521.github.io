...
选择 push 的原因很直接。输入无界，根节点没有"查询结束"这一刻：拉模型的线程会永远阻塞在 source 的空等上；push 让线程在没数据时睡觉，数据到达时被唤醒。输入无界之后，那些在火山模型里不需要单独操心的事情，都会变成无界输入下的问题：

| 有界输入的特点 | 火山模型的现状 | 无界输入的问题 |
|---|---|---|
| `close()` 总会到来 | 查询结束时统一关闭算子 | 无界输入下 `close()` 永不发生，算子必须跨调用保存中间结果 |
| 阻塞算子终将吃完输入 | sort、全量聚合可以等输入结束 | sort、全量聚合永远吃不完 |
...
**Pipeline 回答"数据流到哪里要停"。** 在同一张 DAG 上选择几个分割点，把算子分成一段一段的流水线：火山模型里，阻塞算子就是一个分割点。前面的算子可以边读边处理；遇到 sort、聚合或 hash join 的 build 侧，必须先把数据整理好，才能交给下一段。流系统里，跨网络、需要物化或带 feedback 的地方也会成为分割点；两个分割点之间的算子通常可以合并到同一个线程里执行。Flink 的 operator chain 与 task 边界就是这种分割点，这个定义来自 Stratosphere 系统（Hueske et al., 2012，即 Flink 的前身）。**Pipeline 不是另一种图，而是同一张 DAG 的一种切分方式：DAG 画依赖，pipeline 负责分段。**
...
这个结构和函数调用栈完全同构：调用一层函数，压一帧；返回，弹一帧。时间戳的长度就是嵌套深度，每个坐标是那一层的局部计数器。本质上，系统把函数式程序运行时需要维护的部分状态，压缩进中间结果携带的时空信息里；函数调用不再依赖一条隐藏的控制流，而是把“下一步还要做什么”编码成数据继续向前传递。这也暗合函数式编程的另一面：程序不靠命令式控制流驱动，而是由数据流经一组函数组合子来完成计算。
