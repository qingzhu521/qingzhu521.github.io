...
### 4.2 低水位线（low-watermark）：利用 mark 同步进度
...
### 4.3 pointstamp 和 frontier：每条数据自带进度
...
**④ Timely / Naiad：连估算都不要，精确追踪。** Timely 把"未来还可能出现哪些逻辑时间的工作"直接算出来：每条元组带 `(时间戳, 位置)` 的 pointstamp，系统汇总成 frontier。不需要 slack，原生支持循环和嵌套——这正是 4.3 讲的进度追踪；第一篇的股权穿透迭代也是在这套 `(epoch, iteration)` 坐标下跑的。代价是实现和理解门槛更高，所以它主要是研究系统，而 Flink 把"估算式 watermark"做成了工程主流。
...
### 4.6 越过边界：迟到（lateness）、修订（revision），以及修订的前提
...
三条路线买同一个东西：先给出当前最好的结果，再吸收迟到与更正。而这份能力有一个绕不开的前提——状态：窗口要一直保存累加器（accumulator）或原始记录，join 要保留两侧索引，去重要保留见过的 key，已发布结果的每次变化还可能连带更新下游的聚合、缓存和 sink。revision 半径越大，保留的历史越长，存储与重算越贵；大规模 revision 的状态量至今仍是工程难题。Watermark 与 allowed lateness 合起来划出一条工程边界：边界以内，系统愿意为过去保留 revision 能力；边界以外，状态可以删除，结果才真正封账。
...
状态这个词已经出现了很多次：revision 的前提是它，乱序架构的代价是它，第一节的六个问题里有两个直指它。它到底是什么、归谁管、按什么形状存、多久落一次盘？这就是下一章。
...
状态有了管理者和形状，接下来的问题是多久持久化一次。注意从这里开始，落盘的产物就不再是工作状态本身，而是为故障准备的第二种状态：恢复状态。让 o3 的处理过程在两种粒度下各走一遍。
...
前面说的两条边界——语义边界和恢复边界——是相互独立的，这一点在 Storm、Flink、Naiad 三个系统里都能看出来。

先看 Storm。它的 XOR acker 用固定的少量内存，跟踪每条 tuple 派生出的整棵处理树：哪个分支超时还没确认完，就把那条数据重放一遍。所以 Storm 建立的是恢复边界——它只回答“挂了之后从哪重来”，不回答“事件时间推进到了哪”。至于用户自己放在 bolt 内存或外部数据库里的状态，Storm 一概不管：要不要幂等、要不要事务，都得用户自己解决。

再看 Flink。它的 barrier 沿着数据流切出一个一致的时刻，把算子的 keyed state 和 operator state 存下来，source 同时记下 Kafka offset。所谓“状态上的 exactly-once”，意思是：不管中间挂多少次，每条记录对托管状态的最终影响，都和从来没发生过故障一样。但这只保证状态本身；要做到端到端不丢不重，还得满足两个条件——source 能重放，sink 支持事务或幂等写入。

最后是 Timely/Naiad。它的 frontier 能严格证明“逻辑时间推进到了哪一步”，但进程一消失，内存里的 arrangement 和进度计数就全没了，frontier 自己重建不了。所以 Naiad 另外给有状态的 vertex 定义了 CHECKPOINT/RESTORE 接口，让用户自己保存和恢复状态。

一句话概括三家：frontier 回答“接下来还可能产生什么”，checkpoint 回答“已经攒下的东西怎么恢复”。

拿第五章说的三种状态对照这张图再看一遍，两条边界各管什么就更清楚了——每种状态都有自己对应的边界：
...
输入进度的边界收在语义边界里，工作状态 snapshot 与 offset 收在恢复边界里；输出进度的提交边界则横跨两者——输出提交问题就发生在这个交汇处。

回过头看第一节的六个问题，现在可以一一对号入座了：
...
只有语义边界没有恢复边界，能按时间触发却不能可靠恢复；只有恢复边界没有语义边界，能从故障中回来却永远不知道窗口何时该关。revision 机制对应两条边界之外的第三条事实：完成承诺有时会被迟到数据或业务更正打破，系统必须能撤回旧答案、发布新答案——这就是**结果订正**。
