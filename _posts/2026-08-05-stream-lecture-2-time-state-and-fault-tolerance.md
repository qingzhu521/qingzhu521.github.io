...
同一份订单流在 Timely 里从头到尾走一遍。看代码之前，先把 capability（证）的接口一次摆全——它本来就很小。它存在的理由是：Timely 判断"时间 t 完没完"靠的是**数还有几张 t 的证在外面**，所以证的出生和销毁都必须管死，否则这个数就不可信：

| 类别 | 接口 | 干什么 | 对计数的影响 |
| --- | --- | --- | --- |
| 出生（只有系统能给） | `input.capability()` | 建图时系统发的第一张证（最小时间），一切的起点 | 新证 +1 |
| 出生（随消息来） | `InputCapability` | 算子每收到一条消息，就附带一张那个时间的证 | 新证 +1 |
| 查询 | `cap.time()` | 这张证是什么时间 | 不动 |
| 向未来派生（留底） | `cap.delayed(&t)` | 复印一张到更晚时间，旧的保留 | 新证 +1，旧证不动 |
| 向未来换发（销底） | `cap.downgrade(&t)` | 换成更晚时间，旧的作废 | 新证 +1，旧证 −1 |
| 失败不panic的版本 | `try_delayed` / `try_downgrade` | 同上，但往过去走返回 `None` / `Err`，不 panic | 失败则不动 |
| 算子内转换 | `retain()` / `InputCapability::delayed()` | 把刚收到的消息自带的证，转成自己输出口的证 | 输出口 +1 |
| 复制 | `clone()` | 同一张证再拿一份 | +1 |
| 消亡 | `drop(cap)` | 证被销毁，系统少一张 | −1 |
| 句柄时钟 | `handle.advance_to(t)` | 有序 `Handle` 把内部那张隐含证 downgrade 到 t（外加 flush、关旧 epoch）；证不出句柄，用户拿不到 | 旧 −1、新 +1 |
